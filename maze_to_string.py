...
def calculate(image):
    ...
    # I think converting it to HSV will make some sense, as I have to take care of only one parameter (H)
    color_ranges = {
        "red1": [(0, 120, 50), (10, 255, 255)],  # Lower red range in HSV
        "red2": [(170, 120, 50), (180, 255, 255)],  # Upper red range in HSV
        "green": [(40, 50, 50), (90, 255, 255)],  # Green range in HSV
    }

    def extract_red_green(image):
        image = image.copy()
        hsv = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
        output = np.full_like(image, white) #blank image

        # Basically here I am having a range of red and green values, the problem is that in the main maze, there are variations in the shaedes of grene and red
        # if I just handle one parameter, then because of more saturation, some of the colors of red and blue were getting mapped to Black
        ...
        # BIT Manipulation Striver A2Z SDE sheet, Thank You
    ...
    image = crop_image # Sorry for continous reading of images, I have to do a lot of reformatting and code optimization

    output, mask_red, mask_green = extract_red_green(image)

    # Contouring mean the outlines, I thinks
    contours_red, _ = cv2.findContours(mask_red, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours_green, _ = cv2.findContours(mask_green, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    #My approach is to create a box around red and green colour values, then I will compare the relative positoin between the centroids of the shapes, to get the oritentation of my car
    ...
    #These are my boxes, basically an array having, x,y,w,h coordinates
    ...
    if red_box and green_box:
        ...
        #// means floor division, I want the integer result, it may only differ by 1 pixel and that can be approximated in this project
    ...
    # petition for openCV and matplotlib to adapt same color format 
    ...
    # plt.imshow(output_rgb)
    # plt.axis("off")
    return pathString, scaled_crop_image
